...
你的任务是： 根据用户输入的情况，决定调用哪个智能体（generate_task、optimize_task 或 search_task）来处理任务。  
...
4. 当知识库包含论文摘要和全文时，优先考虑两阶段检索
5. 如果无法确定相关度，table_name保持空字符串
"""
