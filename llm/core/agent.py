...
class SearchAgent:
    ...
    def rag_tool_node(self, state: SearchAgentState) -> Command[Literal['search_agent', 'search_conclude']]:
        ...
        if tool_call['name'] == self.select_tool.name:
            ...
            token_usage = select_dict['raw'].usage_metadata
        ...
        if search_result or not self.use_web:
            return Command(
                update={
                    ...
                    'input_token': token_usage['input_tokens'] if token_usage else 0,
                    'cached_input_token': token_usage['input_token_details']['cache_read'] if token_usage else 0,
                    'output_token': token_usage['output_tokens'] if token_usage else 0,
                },
                goto='search_conclude'
            )
